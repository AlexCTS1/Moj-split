...
// Kljuc zapisa = datum + tocan trening (npr. "2026-08-11__monday").
// Ovo sprjecava da se vjezbe s istim imenom na razlicitim danima (npr. Lat
// pulldown na ponedjeljak I cetvrtak) ikad pomijesaju.
function logKey(dateKey, dayId){ return `${dateKey}__${dayId}`; }
function splitLogKey(key){
  const idx = key.indexOf("__");
  return { date: key.slice(0, idx), dayId: key.slice(idx + 2) };
}

// Migracija starih zapisa (spremljenih samo pod datum, bez dana) u novi oblik.
function migrateLogs(){
  ...
  Object.keys(logs).forEach(k => {
    if(!k.includes("__")){
      const entry = logs[k];
      const dayId = entry.dayId || SPLIT[0].id;
      const newKey = logKey(k, dayId);
      if(logs[newKey]){
        logs[newKey].entries = { ...entry.entries, ...logs[newKey].entries };
      } else {
        logs[newKey] = entry;
      }
      delete logs[k];
...
function findLastSession(dayId, exerciseName, excludeKey){
  ...
  const keys = Object.keys(logs)
    .filter(k => k !== excludeKey && k.endsWith("__" + dayId) && logs[k].entries[exerciseName])
    .sort().reverse();
  if(!keys.length) return null;
  return { date: splitLogKey(keys[0]).date, sets: logs[keys[0]].entries[exerciseName] };
}
...
function renderWorkout(){
  ...
  const key = logKey(todayKey(), day.id);
  const logs = getLogs();
...
function renderHistory(){
  ...
  const rows = Object.keys(logs)
    ...
    .sort().reverse()
    .map(k => ({ key: k, ...splitLogKey(k), rec: logs[k] }));

  const head = document.createElement("div");
  head.className = "day-head";
  head.innerHTML = `<h1 style="color:var(--text)">Povijest</h1><span class="sub">${rows.length} zabiljezenih treninga</span>`;
  ...
  rows.forEach(row => {
    const day = SPLIT.find(d => d.id === row.dayId) || {};
    const details = document.createElement("details");
    details.className = "hist-day";
    details.style.setProperty("--dc", day.color || "var(--text-faint)");
    const exNames = Object.keys(row.rec.entries);
    details.innerHTML = `
      <summary>
        <span class="hist-date">${fmtDate(row.date)}</span>
      ...
      <div class="hist-body">
        ${exNames.map(n => `
          <div class="hist-ex">
            <div class="n">${n}</div>
            <div class="sets">${row.rec.entries[n].map(s => (s.w!=null||s.r!=null) ? `${s.w ?? "?"}kg×${s.r ?? "?"}`:null).filter(Boolean).join("  ·  ") || "—"}</div>
          </div>
        `).join("")}
      </div>
    `;
